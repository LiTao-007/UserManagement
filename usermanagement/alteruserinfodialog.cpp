...
AlterUserInfoDialog::AlterUserInfoDialog(QWidget *parent)
    : QDialog(parent)
{
    setWindowTitle("用户信息修改"); //窗口名称
    setWindowModality(Qt::ApplicationModal); //阻塞除当前窗体之外的所有的窗体
    resize(230,350);

    alterUser_PB = new QPushButton("修改确认");
    ...
    vLayout->addStretch();
    vLayout->addWidget(alterUser_PB);
    setLayout(vLayout);
}

AlterUserInfoDialog::~AlterUserInfoDialog(){
